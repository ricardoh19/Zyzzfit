...
class UserInformationGUI():
    ...
    * Post0. main frame for login is created
    ...
    def createMainFrame(self): 
        ...
        self.weight = Label(self.master, text="Weight",font=("Fixedsys", 15),height = 2, width = 20,borderwidth=0, background='#3E3C3C', foreground='white').grid(row=3,column=0, sticky='w')
        self.weightEntry = Entry(self.master)
        self.weightEntry.grid(row = 3,column=1)
        self.height = Label(self.master, text="Height",font=("Fixedsys", 15),height = 2, width = 20,borderwidth=0, background='#3E3C3C', foreground='white').grid(row=4,column=0, sticky='w')
        ...
        self.chooseGender = OptionMenu(self.master, "one'", "one", "two", "three").grid(row=5,column=1)
        ...
        self.chooseGoal = OptionMenu(self.master, "one'", "one", "two", "three").grid(row=6,column=1)
        ...
        self.saveButton = Button(self.master,text="Save", borderwidth=0).grid(row = 14,column=1, pady=10)
        self.exitButton = Button(self.master,text="Cancel", command=lambda:self.closeWindow(), borderwidth=0).grid(row = 14,column=1, pady=10)
...
def main():
    ...
    userInformationGUIObject = UserInformationGUI(root)
    root.mainloop()
